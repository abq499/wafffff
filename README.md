...
project-waf/
├── docker-compose.yml       # File cấu hình Docker toàn cục
├── requirements.txt         # Các thư viện Python cần thiết
├── proxy/                   # [Layer 1] Code xử lý Reverse Proxy
│   ├── app.py
│   └── Dockerfile
├── model_api/               # [Layer 2] Code xử lý AI Model
│   ├── app.py
│   ├── model.py             # Kiến trúc Bi-LSTM
│   └── Dockerfile
├── dashboard/               # Giao diện giám sát Streamlit
│   └── app.py
├── data/                    # Thư mục chứa dữ liệu
│   ├── labeled_requests.csv # Dataset dùng để train
│   └── requests.jsonl       # Log real-time (Shared Volume)
└── notebooks/               # Các script huấn luyện mô hình
    └── train_simple.py
